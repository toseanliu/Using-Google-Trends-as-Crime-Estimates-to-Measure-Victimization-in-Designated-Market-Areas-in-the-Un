...
plot_texts <- function(df, title, lm_method){
  df <- drop_na(df)
  MVT_text_plot1 <- ggplot(df, aes(x = GT.MVT, y = UCR.MVT, label = DMA))+
  geom_text(size = 2.5, vjust = 1) +
  geom_point(color = "red", size = 1)+
  geom_smooth(method = lm_method) +
  labs(title = "Motor Vehicle Theft") + 
  theme(axis.title.y = element_blank(), axis.title.x = element_blank())

  larceny_text_plot <- ggplot(data = df, aes(x = GT.Larceny, y = UCR.Larceny, label = DMA))+
    geom_text(size = 2.5, vjust = 1) +
    geom_point(color = "red", size = 1)+
    geom_smooth(method = lm_method) +
    labs(title = "Larceny") + 
  ...
  burglary_text_plot <- ggplot(data = df, aes(x = GT.Burglary, y = UCR.Burglary, label = DMA))+
    geom_text(size = 2.5, vjust = 1) +
    geom_point(color = "red", size = 1)+
    geom_smooth(method = lm_method) +
    labs(title = "Burglary") + 
  ...
  rape_text_plot <- ggplot(data = df, aes(x = GT.Rape, y = UCR.Rape, label = DMA))+
    geom_text(size = 2.5, vjust = 1) +
    geom_point(color = "red", size = 1)+
    geom_smooth(method = lm_method) +
    labs(title = "Rape") + 
...
png(file="correlation_dma_text_10_19_plot_lm.png", width = 30, height = 17, unit = "cm",
    res = 200)
plot_texts(final.data.2010_2019, "Figure 5: Scatter Plot of Google Trends and UCR Crimes with DMAs, 2010 to 2019", "lm")
dev.off()

png(file="correlation_dma_text_09_19_plot_loess.png", width = 30, height = 17, unit = "cm",
    res = 200)
plot_texts(final.data.2010_2019, "Figure 5: Scatter Plot of Google Trends and UCR Crimes with DMAs, 2010 to 2019", "loess")
dev.off()

png(file="correlation_dma_text_09_19_plot_rape.png", width = 16, height = 9, unit = "cm",
    res = 200)
ggplot(data = final.data.2010_2019, aes(x = GT.Rape, y = UCR.Rape, label = DMA))+
    geom_text(size = 2.5) +
    geom_smooth(method = "lm") +
    theme(plot.title = element_text(size = 10, hjust = 0.5)) +
    labs(title = "Figure 5: Scatter Plot of Google Trends and UCR Crimes with DMAs, 2010 to 2019") + 
    xlab("Google Trends Rape Estimates") + ylab("UCR Rape Rate")
dev.off()
```
...
arrange_cook <- function(df, title){
  ...
  final_plot <- grid.arrange(arrangeGrob(ucr_gt_mvt_cook_plot, ucr_gt_larceny_cook_plot, 
                             ucr_gt_burglary_cook_plot, ucr_gt_rape_cook_plot, nrow = 2), 
                             legend,
                             widths=c(8, 1),
                             heights=c(200, 1),
...
png(file="cook_09_19_plot.png", width = 30, height = 17, unit = "cm",
    res = 100)
arrange_cook(final.data.2010_2019, 
             "Figure 9: Influence Plot of Google Trends and UCR Crimes, 2010 to 2019")
dev.off()
...
plot_correlation <- function(df, title){
  ...
  gt_ucr_burg <- ggplot(df)+
    ...
    geom_text(x=40, y=70, label = paste("r = ",sprintf("%.3f",cor(df$GT.Burglary,df$UCR.Burglary,use = "complete.obs")),
    ", p-value < ", burglaryp))+ 
    labs(title = "Burglary") +
    theme(axis.title.y = element_blank(), axis.title.x = element_blank())
    xlim(20, 100)
  ...
  gt_ucr_lar <- ggplot(df)+
    ...
    geom_text(x=50, y=80, label = paste("r = ",sprintf("%.3f", cor(df$GT.Larceny,df$UCR.Larceny,use = "complete.obs")),
    ", p-value < ", larcenyp))+ 
    labs(title = "Larceny") +
    theme(axis.title.y = element_blank(), axis.title.x = element_blank())
    xlim(20, 100)
  ...
  gt_ucr_rape <- ggplot(df)+
    ...
    geom_text(x=55, y=45, label = paste("r = ",sprintf("%.3f", cor(df$GT.Rape, df$UCR.Rape, use = "complete.obs")),
    ", p-value < ", rapep))+ 
    labs(title = "Rape") +
    theme(axis.title.y = element_blank(), axis.title.x = element_blank())
    xlim(20, 100)
...
plot_correlation(final.data.2010_2019, 
                 "Figure 1, UCR, and Google Trends Corrlation Scatter Plots")
dev.off()
